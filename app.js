...
function updateChainLabel(){const names=[...document.querySelectorAll(".mchain:checked")].map(x=>x.parentElement.textContent.trim());chainBox.textContent=names.length?(names.length<=2?names.join(", "):names.slice(0,2).join(", ")+" +"+(names.length-2)):"Любая"}
...
function setPreset(mode,el){document.querySelectorAll(".preset").forEach(x=>x.classList.remove("active"));el.classList.add("active");risk.value=mode;if(mode==="conservative"){budget.value="any";note.textContent="Осторожный режим: меньше монет, но выше базовое качество."}if(mode==="balanced"){budget.value="any";note.textContent="Сбалансированный режим: поиск перспективных lowcap без лишней ручной настройки."}if(mode==="aggressive"){budget.value="1";note.textContent="Агрессивный режим: больше ранних и рискованных кандидатов."}}
function resetSearch(){sector.value="auto";budget.value="any";risk.value="balanced";document.querySelectorAll(".mchain").forEach(x=>x.checked=false);updateChainLabel();document.querySelectorAll(".preset").forEach(x=>x.classList.remove("active"));document.querySelector(".preset[data-mode='balanced']").classList.add("active");note.textContent="Сброшено. Выбери сектор, сети и нажми поиск.";showToast("Сброшено")}
...
function coinCard(c,watchMode){const div=document.createElement("div");div.className="coin"+(watchMode?" watchCompact":"");const cap=c.market_cap||c.fdv||0,risks=riskList(c),score=c._score||0,ch=c.price_change_percentage_24h||0;const fallback=(c.symbol||"?").slice(0,3).toUpperCase();const img=c.image?`<img class="avatar" src="${c.image}" onerror="this.outerHTML='<div class=&quot;avatar fallback&quot;>${fallback}</div>'">`:`<div class="avatar fallback">${fallback}</div>`;div.innerHTML=`<div class="coinTop">${img}<div style="min-width:0;flex:1"><div class="coinName">${c.name||"Unknown"}</div><div class="coinSub">${(c.symbol||"").toUpperCase()} · ${c._source||"Multi"}</div></div><div class="score ${score<45?"bad":score<70?"mid":""}">${score}</div></div><div class="badges"><span class="badge ${risks.length?"warn":"good"}">Риски: ${risks.length}</span><span class="badge">Сеть: ${c._dexChain||"market"}</span><span class="badge">${narrative(c)}</span><span class="badge">Источник: ${c._source||"multi"}</span></div><div class="sourceLine">Факты подтверждаются ссылками в ячейках. Если данных нет — приложение честно показывает «нет данных».</div><div class="analysisGrid">${metric("Капитализация",fmt(cap),metricProof(c,"cap"))}${metric("Объём 24ч",fmt(c.total_volume),metricProof(c,"volume"))}${metric("Рост цены 24ч",`<span class="${ch>=0?"goodText":"badText"}">${pct(ch)}</span>`,metricProof(c,"price"))}${metric("Ликвидность",liqText(c),metricProof(c,"liquidity"))}${metric("Возраст проекта",c.age_days?`${c.age_days}д`:"нет данных",metricProof(c,"age"))}${metric("Соцсети",c.website?"сайт найден":"нет данных",c.website?`<a class="proofLink" target="_blank" href="${c.website}">открыть сайт</a>`:"")}${metric("Комьюнити",trendText(c),metricProof(c,"volume"))}${metric("Фонды / backing",fundsSignal(c),c.backingUrl?`<a class="proofLink" target="_blank" href="${c.backingUrl}">подтверждение</a>`:"")}${metric("Экосистема",ecosystem(c),metricProof(c,"chain"))}${metric("Нарратив / сектор",narrative(c),metricProof(c,"sector"))}</div><div class="explain"><b>Почему AI выбрал:</b> капитализация ${fmt(cap)}, ликвидность ${liqText(c)}, объём 24ч ${fmt(c.total_volume)}, движение ${pct(ch)}. ${risks.length?("Риски: "+risks.join(", ")):"Критичных рисков по доступным данным нет."}</div>${tradeBox(c)}<div class="links">${c.url?`<a class="linkBtn" target="_blank" href="${c.url}">DexScreener</a>`:""}<a class="linkBtn" target="_blank" href="https://coinmarketcap.com/search/?q=${encodeURIComponent(c.name||c.symbol||"")}">CoinMarketCap</a><a class="linkBtn" target="_blank" href="https://www.coingecko.com/en/search?query=${encodeURIComponent(c.name||c.symbol||"")}">CoinGecko</a></div>${watchMode?positionBlock(c):""}${watchMode?`<button class="danger watchBtn" onclick="event.stopPropagation();removeWatch('${c.id}')">Удалить</button>`:`<button class="secondary watchBtn">В избранное</button>`}`;if(!watchMode)div.querySelector(".watchBtn").onclick=()=>addWatch(c);if(watchMode)div.addEventListener("click",()=>div.classList.toggle("open"));return div}
