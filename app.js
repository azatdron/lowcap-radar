...
/* lr24 safe cleanup over v23 */
function lr24T(ru,en){try{if(typeof lrLang!=="undefined"&&lrLang==="en")return en;const b=document.body.innerText||"";if(b.includes("Start scan")||b.includes("Found"))return en}catch(e){}return ru}
function lr24Fmt(n){n=Number(n||0);if(!isFinite(n))n=0;if(Math.abs(n)>=1e9)return"$"+(n/1e9).toFixed(1)+"B";if(Math.abs(n)>=1e6)return"$"+(n/1e6).toFixed(1)+"M";if(Math.abs(n)>=1e3)return"$"+(n/1e3).toFixed(0)+"K";if(Math.abs(n)<0.01&&n!==0)return"$"+n.toPrecision(3);return"$"+n.toFixed(2)}
function lr24AddTopChips(card,c){try{if(!card||!c)return;const host=card.querySelector(".chips,.badges,.tagRow");if(!host)return;const txt=(host.textContent||"").toLowerCase();function add(v){if(!v)return;const key=v.split(" ")[0].toLowerCase();if(txt.includes(key))return;const el=document.createElement("span");el.className="chip compactChipV24";el.textContent=v;host.appendChild(el)}if(c.total_volume&&!txt.includes("объём")&&!txt.includes("vol"))add(lr24T("Объём ","Vol ")+lr24Fmt(c.total_volume));if(c.age_days&&!txt.includes("возраст")&&!txt.includes("age"))add(lr24T("Возраст ","Age ")+String(c.age_days)+lr24T("д","d"));const dex=c.dex||c.exchange||c.chain||"";if(dex&&!txt.includes(String(dex).toLowerCase()))add(String(dex).slice(0,18))}catch(e){}}
function lr24Cleanup(){try{document.querySelectorAll("[data-coin-key].open").forEach(card=>{const k=card.getAttribute("data-coin-key");const c=(typeof coinStore!=="undefined"&&coinStore.get(k))||(typeof watchItems==="function"?watchItems().find(x=>key(x)===k):null)||(typeof lastResults!=="undefined"?lastResults.find(x=>key(x)===k):null);lr24AddTopChips(card,c);const d=card.querySelector(".details")||card;d.querySelectorAll(".compactExtraV20,.compactExtraV21,.compactExtraV24,.posSummaryV19,.posSummaryV20,.posSummaryV21,.avgBlockV18,.tradeBox,.aiTrade,.tradeLayer").forEach(el=>el.remove());const links=d.querySelector(".links,.compactLinksV19,.compactLinksV20,.compactLinksV21");if(links){links.querySelectorAll("a").forEach(a=>{const h=(a.href||"").toLowerCase();if(h.includes("dexscreener"))a.textContent="Dex";else if(h.includes("coinmarketcap"))a.textContent="CMC";else if(h.includes("coingecko"))a.textContent="CG"})}})}catch(e){}}
document.addEventListener("DOMContentLoaded",()=>{setTimeout(lr24Cleanup,150);setTimeout(lr24Cleanup,650)});document.addEventListener("click",()=>{setTimeout(lr24Cleanup,120);setTimeout(lr24Cleanup,500)});
