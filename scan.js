...
function budgetOk(c,b){return !b||b==="any"||num(c.current_price)<=Number(b)}function qualityOk(c,r){
  ...
  if(r==="conservative") return liq>=25000 && vol>=25000;
  return liq>=10000 && vol>=3000;
}
function sectorOk(c,sector){ if(String(sector||'').includes(',')){const parts=String(sector).split(',').map(x=>x.trim()).filter(Boolean);return parts.includes('auto')||parts.some(s=>sectorOk(c,s));} if(sector==="auto")return true;const t=(c.name+" "+c.symbol+" "+(c.description||"")+" "+(c.categories||[]).join(" ")).toLowerCase();return (TERMS[sector]||[]).some(x=>t.includes(x))}
function dedupe(list){const m=new Map();for(const c of list){const k=norm(c.symbol)+":"+norm(c.name);const old=m.get(k);const w=num(c.liquidity)*3+num(c.total_volume)+num(c.market_cap||c.fdv)/300+(c._source==="CoinMarketCap"?50000:0)+(c._source==="CoinGecko"?30000:0);const ow=old?num(old.liquidity)*3+num(old.total_volume)+num(old.market_cap||old.fdv)/300+(old._source==="CoinMarketCap"?50000:0)+(old._source==="CoinGecko"?30000:0):-1;if(!old||w>ow)m.set(k,c)}return[...m.values()]}
async function fetchCoinGecko(){const out=[];try{const r=await fetch(`${CG_BASE}/coins/markets?vs_currency=usd&order=volume_desc&per_page=250&page=1&sparkline=false&price_change_percentage=24h,7d,30d`,{headers:{accept:"application/json"}});if(!r.ok)return out;const arr=await r.json();for(const x of arr)out.push({id:x.id,name:x.name,symbol:x.symbol,image:x.image,current_price:x.current_price,market_cap:x.market_cap,fdv:x.fully_diluted_valuation,total_volume:x.total_volume,price_change_percentage_24h:x.price_change_percentage_24h,price_change_percentage_7d:x.price_change_percentage_7d_in_currency,price_change_percentage_30d:x.price_change_percentage_30d_in_currency,liquidity:0,age_days:0,_source:"CoinGecko",_dexChain:"market",dex:"market",url:`https://www.coingecko.com/en/coins/${x.id}`})}catch(e){}return out}
async function fetchCMC(){const out=[];const key=process.env.CMC_API_KEY;if(!key)return out;try{const r=await fetch(`${CMC_BASE}/cryptocurrency/listings/latest?limit=500&convert=USD`,{headers:{"X-CMC_PRO_API_KEY":key}});if(!r.ok)return out;const j=await r.json();for(const x of j.data||[]){const q=x.quote?.USD||{};out.push({id:String(x.id),name:x.name,symbol:x.symbol,current_price:q.price,market_cap:q.market_cap,fdv:q.fully_diluted_market_cap,total_volume:q.volume_24h,price_change_percentage_24h:q.percent_change_24h,price_change_percentage_7d:q.percent_change_7d,price_change_percentage_30d:q.percent_change_30d,liquidity:0,age_days:0,_source:"CoinMarketCap",_dexChain:"market",dex:"market",url:`https://coinmarketcap.com/currencies/${String(x.slug||x.name).toLowerCase().replaceAll(" ","-")}/`})}}catch(e){}return out}
async function fetchDex(chains,sector,risk){const out=[];const terms=(TERMS[sector]||TERMS.auto).slice(0,risk==="aggressive"?10:6);for(const chain of chains){for(const q of terms){try{const r=await fetch(`${DEX_BASE}/search?q=${encodeURIComponent(chain==="auto"?q:chain+" "+q)}`);if(!r.ok)continue;const j=await r.json();for(const p of j.pairs||[]){if(chain!=="auto"&&String(p.chainId).toLowerCase()!==CHAIN[chain])continue;out.push({id:String(p.chainId)+"-"+String(p.pairAddress),name:p.baseToken?.name||"Unknown",symbol:p.baseToken?.symbol||"?",image:p.info?.imageUrl||"",current_price:num(p.priceUsd),market_cap:num(p.marketCap),fdv:num(p.fdv),total_volume:num(p.volume?.h24),price_change_percentage_24h:num(p.priceChange?.h24),liquidity:num(p.liquidity?.usd),age_days:p.pairCreatedAt?Math.floor((Date.now()-p.pairCreatedAt)/86400000):0,_source:"DexScreener",_dexChain:String(p.chainId).toLowerCase(),dex:p.dexId||"",pair:p.pairAddress||"",tokenAddress:p.baseToken?.address||"",url:p.url||"",website:(p.info?.websites&&p.info.websites[0]?.url)||""})}}catch(e){}}}return out}
