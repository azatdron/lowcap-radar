...
function proof(c,type){const ds=c.url||"",cg=`https://www.coingecko.com/en/search?query=${encodeURIComponent(c.name||c.symbol||"")}`,cmc=`https://coinmarketcap.com/search/?q=${encodeURIComponent(c.name||c.symbol||"")}`;if(["liquidity","volume","chain","age"].includes(type)&&ds)return`<a class="proofLink" target="_blank" href="${ds}"></a>`;if(["cap","price"].includes(type))return`<a class="proofLink" target="_blank" href="${cg}"></a>`;return`<a class="proofLink" target="_blank" href="${cmc}"></a>`}
...
function tradeBox(c){const p=Number(c.current_price||0),score=Number(c._score||0),r=riskCount(c),ch=Number(c.price_change_percentage_24h||0),liq=Number(c.liquidity||0),vol=Number(c.total_volume||0);if(!p)return"";const m=r>2?.18:r>0?.14:.1;const entryLow=p*(ch>15?.88:ch>5?.94:ch<0?.97:.95);const entryHigh=p*(ch>15?.96:ch>5?1.01:ch<0?1.02:1);const stop=p*(1-m*1.45);const tp1=p*(score>=75?1.28:score>=55?1.18:1.1);const tp2=p*(score>=75?1.65:score>=55?1.38:1.22);let rr=(tp1-p)/(p-stop);if(!isFinite(rr)||rr<0)rr=0;let note=``;if(liq<25000)note+=" Ликвидность слабая.";if(vol>liq*5&&liq>0)note+=" Объём сильно выше ликвидности.";return`<div class="tradeBox"><div class="tradeTitle"></div><div class="tradeGrid"><div class="tradeItem"><div class="tLabel"></div><div class="tValue">${priceFmt(entryLow)} – ${priceFmt(entryHigh)}</div></div><div class="tradeItem"><div class="tLabel"></div><div class="tValue">${priceFmt(stop)}</div></div><div class="tradeItem"><div class="tLabel"></div><div class="tValue">${priceFmt(tp1)}</div></div><div class="tradeItem"><div class="tLabel"></div><div class="tValue">${priceFmt(tp2)}</div></div><div class="tradeItem"><div class="tLabel">Risk/Reward</div><div class="tValue">1:${rr.toFixed(1)}</div></div></div><div class="tradeExplain">${note}</div><div class="tradeWarn"></div></div>`}
