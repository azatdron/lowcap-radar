...
/* v24 full over stable v23 */
function v24fullT(ru,en){try{if(typeof lrLang!=="undefined"&&lrLang==="en")return en;const b=document.body.innerText||"";if(b.includes("Start scan")||b.includes("Found"))return en}catch(e){}return ru}
function v24fullFmt(n){n=Number(n||0);if(!isFinite(n))n=0;if(Math.abs(n)>=1e9)return"$"+(n/1e9).toFixed(1)+"B";if(Math.abs(n)>=1e6)return"$"+(n/1e6).toFixed(1)+"M";if(Math.abs(n)>=1e3)return"$"+(n/1e3).toFixed(0)+"K";if(Math.abs(n)<0.01&&n!==0)return"$"+n.toPrecision(3);return"$"+n.toFixed(2)}
function v24fullEnsureTopChips(card,c){try{let host=card.querySelector(".chips,.badges,.tagRow");if(!host||!c)return;const text=(host.textContent||"").toLowerCase();const add=(label,value)=>{if(!value||text.includes(label.toLowerCase()))return;const el=document.createElement("span");el.className="chip compactChipV24";el.textContent=label+" "+value;host.appendChild(el)};add(v24fullT("Объём","Vol"),c.total_volume?v24fullFmt(c.total_volume):"");add(v24fullT("Возраст","Age"),c.age_days?String(c.age_days)+v24fullT("д","d"):"");const dex=c.dex||c.exchange||c.chain||"";if(dex&&!text.includes(String(dex).toLowerCase())){const el=document.createElement("span");el.className="chip compactChipV24";el.textContent=String(dex).slice(0,18);host.appendChild(el)}}catch(e){}}
function v24fullCleanup(){try{document.querySelectorAll("[data-coin-key].open").forEach(card=>{const k=card.getAttribute("data-coin-key");const c=(typeof coinStore!=="undefined"&&coinStore.get(k))||(typeof watchItems==="function"?watchItems().find(x=>key(x)===k):null)||(typeof lastResults!=="undefined"?lastResults.find(x=>key(x)===k):null);if(c)v24fullEnsureTopChips(card,c);const details=card.querySelector(".details")||card;details.querySelectorAll(".compactExtraV20,.compactExtraV21,.compactExtraV24").forEach(el=>el.remove());const links=details.querySelector(".links,.compactLinksV19,.compactLinksV20,.compactLinksV21");if(links){links.querySelectorAll("a").forEach(a=>{const href=(a.href||"").toLowerCase();if(href.includes("dexscreener"))a.textContent="Dex";else if(href.includes("coinmarketcap"))a.textContent="CMC";else if(href.includes("coingecko"))a.textContent="CG"})}details.querySelectorAll(".posSummaryV19,.posSummaryV20,.posSummaryV21,.avgBlockV18").forEach(el=>el.remove());details.querySelectorAll(".tradeBox,.aiTrade,.tradeLayer").forEach(el=>el.remove());const pos=details.querySelector(".positionPanelV19,.positionPanelV20,.positionPanelV21,.tradePositionV18");if(pos)pos.dataset.stablePosition="1"})}catch(e){}}
document.addEventListener("DOMContentLoaded",()=>{setTimeout(v24fullCleanup,150);setTimeout(v24fullCleanup,650)})
document.addEventListener("click",()=>{setTimeout(v24fullCleanup,120);setTimeout(v24fullCleanup,500)})
